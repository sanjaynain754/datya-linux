...
Optional capability profiles are catalogued in `profiles/catalog.toml` and can be installed from the current Debian repositories with `tools/datya-profile.sh`. Datya does not mix Kali repositories into its Debian base or claim that every Linux feature is installed; each capability must carry provenance, license, architecture, privilege, network, test, and uninstall metadata.
